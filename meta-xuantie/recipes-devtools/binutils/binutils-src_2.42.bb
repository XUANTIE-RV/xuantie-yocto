...
SRCREV = "${@bb.utils.contains('REPO_LOCATE_CONFIG', 'external', \
    'f7bd729ea7de193ae9cc48ab04baa37e261348db', \
    '${XUANTIE_LINUX_TAG}', d)}"

SRC_URI = "${@bb.utils.contains('REPO_LOCATE_CONFIG', 'external', \
    'git://gitee.com/xuantie-yocto/binutils-gdb.git;branch=develop;protocol=https', \
    'git://gitlab.alibaba-inc.com/thead/binutils-gdb.git;branch=binutils-2_42-xt;protocol=ssh', d)}"
...
do_install:append() {
    rm -rf ${D}${libdir}/bfd-plugins/libdep.so
}
